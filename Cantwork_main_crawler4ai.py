...
from crawl4ai import Crawler4AIg
import time
import random

# Create an output directory for the scraped transcripts
output_dir = "scraped_transcripts_crawler4ai"
os.makedirs(output_dir, exist_ok=True)

# URLs to scrape (keeping your original URLs)
urls = [
...
async def scrape_url(url):
    """Scrape content from a URL using crawler4ai."""
    try:
        print(f"\n🔎 Visiting: {url}")
        ...
        page_content = await crawler.get_page_content(url)

        # Extract transcript if available
        transcript_text = ""
        transcript_button = page_content.find_element_by_text("Show full transcript")
        if transcript_button:
            ...
            for selector in transcript_selectors:
                transcript_element = page_content.find_element(selector)
                if transcript_element:
                    transcript_text = transcript_element.text
                    break

        # Extract paragraphs
        paragraphs = page_content.find_elements("p")
        page_paragraphs_text = "\n\n".join([p.text for p in paragraphs if p.text.strip()])
